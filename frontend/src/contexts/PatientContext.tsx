...
type PatientAction =
  ...
  | { type: 'APPEND_PATIENTS'; payload: Patient[] };
...
function patientReducer(state: any, action: PatientAction) {
  switch (action.type) {
    ...
    case 'APPEND_PATIENTS':
      return { ...state, patients: [...state.patients, ...action.payload], isLoading: false };
    default:
      return state;
  }
}
...
export function PatientProvider({ children }: PatientProviderProps) {
  ...
  const loadMorePatients = async (): Promise<void> => {
    try {
      if (state.isLoading) return;
      dispatch({ type: 'SET_LOADING', payload: true });

      const nextPage = (state.searchQuery.page || 1) + 1;
      const searchParams = new URLSearchParams();
      const finalQuery = { ...state.searchQuery, page: nextPage };
      ...
      if (response.patients.length > 0) {
        dispatch({ type: 'APPEND_PATIENTS', payload: response.patients });
        dispatch({ type: 'SET_SEARCH_QUERY', payload: { page: nextPage } });
      } else {
        dispatch({ type: 'SET_LOADING', payload: false });
      }
    } catch (error) {
      dispatch({ type: 'SET_ERROR', payload: error instanceof Error ? error.message : 'Failed to load more patients' });
    }
  };
