...
async function apiCall<T>(
  ...
  if (!response.ok) {
    // Handle authentication errors (401 Unauthorized or 400 with auth issues)
    ...
    // Check if it's a 400 error with "Unauthorized" message (token expired)
    if (
      response.status === 400 &&
      error.error?.details?.some((d: any) => d.message === 'Unauthorized')
    ) {
      handleAuthError();
      throw new Error('Session expired. Please login again.');
    }
...
export function transformWizardDataToPayload(formData: any): CreatePatientPayload {
  ...
  return {
    ...
    // Section 2
    hospitalRecordNumber: section2.hospitalRecordNumber || `MR-${Date.now()}`,
    nik: section2.nik || '',
    name: section2.name || '',
...
/**
 * Default export object with all API methods
 */
const patientApi = {
  ...
  transformWizardData: transformWizardDataToPayload,
};
