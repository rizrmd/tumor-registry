  ...
  const fetchResearchData = async () => {
    try {
      ...
      const researchData: ResearchData[] = patients.map((patient: any) => {
        ...
        return {
          id: patient.id,
          inamsosRecordNumber: patient.inamsosRecordNumber || '-',
              ...
              {filteredData.map((record) => (
                <tr key={record.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900">{record.inamsosRecordNumber}</div>
                    <div className="text-xs text-gray-500">{record.hospitalRecordNumber}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${record.grade.includes('High') || record.grade.includes('high') ? 'bg-red-100 text-red-800' :
