  ...
  const handleSubmitAdd = async (e: React.FormEvent) => {
    ...
    try {
      await centersService.createCenter({
        ...
        mrPrefix: formData.mrPrefix ? formData.mrPrefix.toUpperCase() : undefined,
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredCenters.map((center) => (
                <tr key={center.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-500">{center.mrPrefix || '-'}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
            ...
            <form onSubmit={handleSubmitAdd}>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={formData.mrPrefix}
                    onChange={(e) =>
                      setFormData({ ...formData, mrPrefix: e.target.value.toUpperCase() })
                    }
                    placeholder="Contoh: IND"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                  />
                  <p className="text-xs text-gray-500 mt-1">Kode prefix untuk nomor rekam medis (default: Kode Pusat)</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Alamat
