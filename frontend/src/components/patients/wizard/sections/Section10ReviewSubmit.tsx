...
export function Section10ReviewSubmit() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // NEW: Upload Radiology Images (Deferred)
      const radiologyModalities = formData.section4?.radiologyModalities;
      if (radiologyModalities && radiologyModalities.length > 0 && response.data?.id) {
        console.log('Processing radiology uploads...');
        // Import service dynamically or use existing import if available
        // We need medicalImagingService here
        const { default: medicalImagingService, ImageCategory, MedicalImageType } = await import('@/services/medical-imaging.service');

        for (const modality of radiologyModalities) {
          if (modality.images && modality.images.length > 0) {
            for (const img of modality.images) {
              if (img.file) {
                try {
                  await medicalImagingService.uploadImage(img.file, {
                    patientId: response.data.id,
                    category: ImageCategory.RADIOLOGY, // Or appropriate category
                    imageType: MedicalImageType[modality.modality as keyof typeof MedicalImageType] || MedicalImageType.XRAY,
                    modality: modality.modality,
                    studyDate: modality.studyDate,
                    findings: modality.findings
                  });
                  console.log(`Uploaded ${modality.modality} image: ${img.fileName}`);
                } catch (err) {
      ...
      // NEW: Upload Pathology Files (Deferred)
      const pathologyFiles = formData.section4?.pathologyFiles;
      if (pathologyFiles && pathologyFiles.length > 0 && response.data?.id) {
        console.log('Processing pathology uploads...');
        const { default: medicalImagingService, ImageCategory, MedicalImageType } = await import('@/services/medical-imaging.service');

        for (const pFile of pathologyFiles) {
          if (pFile.file) {
            try {
              await medicalImagingService.uploadImage(pFile.file, {
                patientId: response.data.id,
                category: ImageCategory.PATHOLOGY,
                imageType: MedicalImageType.PATHOLOGY,
                description: 'Initial pathology report upload'
              });
              console.log(`Uploaded pathology file: ${pFile.fileName}`);
            } catch (err) {
              console.error(`Failed to upload ${pFile.fileName}`, err);
            }
          }
        }
      }
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {sections.map((section) => (
          <div
            key={section.number}
            className={`bg-white rounded-lg p-5 border-2 transition-all ${section.isValid
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4 flex-1">
                ...
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${section.isValid ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                    }`}
                >
                  {section.number}
                </div>
      ...
      <div className="bg-white rounded-lg p-6 border border-gray-200">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            disabled={!allValid}
            className={`px-8 py-3 rounded-md font-semibold text-white transition-all ${allValid
