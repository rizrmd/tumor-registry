...
export default function PatientEntryForm({ onSuccess }: PatientEntryFormProps) {
  ...
  const transformFormDataToDto = (data: Record<string, any>) => {
    return {
      hospitalRecordNumber: data.hospitalRecordNumber || `RM-${Date.now()}`,
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-colors ${mode === 'quick'
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-colors ${mode === 'full'
                  ? 'bg-green-600 text-white'
                  : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                }`}
            >
