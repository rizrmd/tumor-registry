...
## 🚫 **CRITICAL: NO HARDCODED DATA POLICY**
...
- ❌ Hardcoded patient IDs (`demo-patient-001`, `test-patient-123`, etc.)
- ❌ Hardcoded center IDs or names in code
- ❌ Hardcoded user credentials in components
- ❌ Hardcoded WHO classification IDs
- ❌ Hardcoded location IDs (bone/soft tissue)
- ❌ Demo pages with static data (`/demo/*` routes)
- ❌ Test data embedded in production components
- ❌ Placeholder strings in place of dynamic data

### **Required Practices:**

1. **Always Query Database:**
   ```typescript
   // ❌ WRONG
   const centerId = 'DKI001';

   // ✅ CORRECT
   const centers = await fetchCenters();
   const selectedCenter = centers[0]?.id;
   ```

2. **Use Route Parameters for IDs:**
   ```typescript
   // ❌ WRONG - Demo pages
   /demo/follow-up-calendar → Hardcoded patient "demo-patient-001"

   // ✅ CORRECT - Dynamic routes
   /patients/[id]/follow-up → Dynamic patient from database
   ```

3. **Props Over Constants:**
   ```typescript
   // ❌ WRONG
...
5. **Seeders for Test Data:**
   ```bash
   # ✅ CORRECT - Use seeders, not hardcoded data
   bun run scripts/seed-dummy-data.ts
   ```

### **What IS Allowed:**

✅ **Enum values and constants for validation:**
```typescript
const ENNEKING_STAGES = ['IA', 'IB', 'IIA', 'IIB', 'III'];
const TUMOR_GRADES = ['Low-grade (G1)', 'High-grade (G2)'];
```

✅ **Static configuration in JSON files:**
```json
// config/centers.json
{ "name": "RSUP Dr. Cipto", "code": "DKI001" }
```

✅ **Default values with fallbacks:**
```typescript
const pageSize = params.limit || 10; // ✅ Default is OK
```

✅ **Placeholder text for UI labels:**
```typescript
<input placeholder="Enter patient name" /> // ✅ UI text is OK
```

### **Enforcement:**

- Any PR with hardcoded IDs/names in components will be REJECTED
- Demo pages (`/demo/*`) are BANNED
- Code review must check for hardcoded values
- Use linting rules to detect hardcoded strings

### **Migration Complete:**

✅ **2025-12-28**: Removed all demo pages (`/demo/follow-up-calendar`, `/demo/msts-calculator`)
✅ **2025-12-28**: Created seeder for 5 dummy patients with diverse data
✅ **2025-12-28**: Seeded 21 designated musculoskeletal tumor centers

**Next Steps:**
- Implement patient-specific routes: `/patients/[id]/follow-up`, `/patients/[id]/msts`
- All components must receive patient ID as prop or route param
