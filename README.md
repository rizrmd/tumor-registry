...
├── desktop/                 # Wails-based desktop application
│   ├── cmd/runner/          # Standalone runner for offline mode
│   ├── pkg/process/         # Go-based process manager for Postgres and Backend
│   ├── scripts/             # Setup and binary management scripts
...
## 💻 Desktop Application (Offline Mode)

INAMSOS includes a desktop wrapper built with **Wails**, enabling offline data collection and synchronization.

### Key Features:
- **Standalone Runner**: Bundles PostgreSQL and the NestJS backend into a single executable.
- **Offline Sync**: Automatically queues mutations when offline and syncs with the remote server when a connection is available.
- **Native Experience**: Native window management and file system access.
