...
var HttpExceptionFilter_1, ValidationExceptionFilter_1;
...
let HttpExceptionFilter = HttpExceptionFilter_1 = class HttpExceptionFilter {
    constructor(configService) {
        ...
        if (status === common_1.HttpStatus.NOT_FOUND && !request.url.startsWith('/api/')) {
            const frontendPath = (0, path_1.join)(process.cwd(), '../frontend/out');
            const indexPath = (0, path_1.join)(frontendPath, 'index.html');
            if (fs.existsSync(indexPath)) {
                return response.type('text/html').send(fs.readFileSync(indexPath));
            }
        }
