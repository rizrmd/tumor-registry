...
try {
    ...
    fsDebug.appendFileSync('d:/backend_launch_debug.txt', logVal);

    // Override console.error to capture runtime errors
    const errLog = fsDebug.createWriteStream('d:/backend_runtime_error.log', { flags: 'a' });
    const origErr = console.error;
    console.error = (...args) => {
        errLog.write(`[${new Date().toISOString()}] ERROR: ${args.map(a => String(a)).join(' ')}\n`);
        origErr(...args);
    };
    const logLog = fsDebug.createWriteStream('d:/backend_runtime_logo.log', { flags: 'a' });
    const origLog = console.log;
    console.log = (...args) => {
        logLog.write(`[${new Date().toISOString()}] LOG: ${args.map(a => String(a)).join(' ')}\n`);
        origLog(...args);
    }
} catch (e) { }
