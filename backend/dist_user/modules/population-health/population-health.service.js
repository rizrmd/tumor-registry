...
var PopulationHealthService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.PopulationHealthService = void 0;
const common_1 = require("@nestjs/common");
const prisma_service_1 = require("@/database/prisma.service");
let PopulationHealthService = PopulationHealthService_1 = class PopulationHealthService {
    constructor(prisma) {
        this.prisma = prisma;
        this.logger = new common_1.Logger(PopulationHealthService_1.name);
    }
    async getPopulationHealthOverview(province, regency) {
        try {
            const where = {};
            if (province) {
            ...
            const [totalPopulation, cancerIncidenceRate, mortalityRate, screeningCoverage, earlyDetectionRate, topCancerTypes, ageDistribution, genderDistribution, socioeconomicData, healthcareAccess,] = await Promise.all([
    ...
    async getCancerIncidenceByRegion(level = 'province') {
        try {
    ...
    async getScreeningProgramEffectiveness() {
        try {
        ...
        try {
            ...
            return {
                ...
                summary: {
                    ...
                    modifiableFactors: riskFactors.length,
    ...
    async getPopulationProjections(years = 10) {
        try {
            const currentYear = new Date().getFullYear();
            const projections = [];
            for (let i = 0; i <= years; i++) {
                const targetYear = currentYear + i;
                const projectedPopulation = 270000000 + (i * 1500000);
                const projectedCancerCases = Math.floor(projectedPopulation * 0.0015);
                projections.push({
                    year: targetYear,
    ...
    async getHealthEconomicAnalysis() {
        try {
            const economicData = {
                directCosts: {
                    treatment: 2500000000,
                    diagnostics: 500000000,
                    medications: 800000000,
                    hospitalization: 600000000,
                },
                indirectCosts: {
                    productivityLoss: 1500000000,
                    caregiverCosts: 300000000,
                    transportation: 100000000,
                },
                preventionSavings: {
                    smokingCessation: 450000000,
                    vaccinationPrograms: 280000000,
                    earlyDetection: 620000000,
                },
                costEffectiveness: {
                    screeningPrograms: [
    ...
    async generatePopulationHealthReport(reportType, filters = {}) {
        try {
            const reportData = {
                metadata: {
                    reportType,
    ...
    async getTotalPopulation(where) {
        return 270000000;
    }
    async getCancerIncidenceRate(where) {
        return 156.2;
    }
    async getMortalityRate(where) {
        return 92.4;
    }
    async getScreeningCoverage(where) {
        return 23.5;
    }
    async getEarlyDetectionRate(where) {
        return 42.8;
    }
    async getTopCancerTypes(where) {
        return [
    ...
    async getAgeDistribution(where) {
        return [
    ...
    async getHealthcareAccessData(where) {
        return {
            insurance: 85.2,
    ...
    generateExecutiveSummary(reportData, reportType) {
...
exports.PopulationHealthService = PopulationHealthService = PopulationHealthService_1 = __decorate([
