...
let PerformanceService = PerformanceService_1 = class PerformanceService {
    ...
    async getApplicationPerformanceMetrics() {
        try {
            ...
            const [memoryUsage, cpuUsage, responseTimeMetrics, errorMetrics, cacheMetrics, databaseMetrics,] = await Promise.all([
        ...
        return [
            {
                ...
                bloatPercentage: 35,
                estimatedBloatSize: 256 * 1024 * 1024,
            },
            {
                tableName: 'medical_records',
                bloatPercentage: 28,
                estimatedBloatSize: 128 * 1024 * 1024,
            },
        ];
    }
    ...
    getQueryOptimizationSuggestion(query) {
    ...
    calculateOverallImpact(recommendations) {
        ...
        if (highPriority > 2)
            return 'High - Critical performance improvements available';
        if (highPriority > 0)
            return 'Medium-High - Significant improvements possible';
    ...
    async getResponseTimeMetrics(since) {
        return {
    ...
    calculateApplicationHealthScore(metrics) {
        ...
        if (metrics.memoryUsage.percentageUsed > 80)
            score -= 20;
        else if (metrics.memoryUsage.percentageUsed > 60)
            score -= 10;
        if (metrics.responseTimeMetrics.p95 > 500)
            score -= 20;
        else if (metrics.responseTimeMetrics.p95 > 300)
            score -= 10;
        if (metrics.errorMetrics.errorRate > 0.05)
            score -= 25;
        else if (metrics.errorMetrics.errorRate > 0.02)
            score -= 10;
        if (metrics.cacheMetrics.hitRate > 0.9)
            score += 5;
        else if (metrics.cacheMetrics.hitRate < 0.7)
            score -= 15;
        return Math.max(0, Math.min(100, score));
    }
    async optimizePatientQueries() {
        return {
            optimizedCount: 5,
            improvements: [
                {
    ...
    async optimizeAnalyticsQueries() {
        return {
            optimizedCount: 3,
            improvements: [
                {
    ...
    async optimizeReportingQueries() {
        return {
            optimizedCount: 6,
            improvements: [
                {
    ...
    calculateOverallPerformanceGain(results) {
        const totalOptimized = Object.values(results).reduce((sum, result) => sum + (result.optimizedCount || 0), 0);
        if (totalOptimized > 15)
            return '70-85% improvement in overall performance';
        if (totalOptimized > 10)
            return '50-70% improvement in overall performance';
        return '30-50% improvement in overall performance';
    }
    generateQueryRecommendations(results) {
        const recommendations = [];
        const totalOptimized = Object.values(results).reduce((sum, result) => sum + (result.optimizedCount || 0), 0);
        recommendations.push({
            category: 'monitoring',
