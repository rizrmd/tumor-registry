...
const ROLES = [
  ...
  },
  {
    code: 'HEAD_OF_CENTER',
    name: 'Head of Center',
    description: 'Center leadership with input and data request capabilities',
    level: 2,
  },
  {
    code: 'OBSERVER',
    name: 'Observer',
    description: 'Read-only access to local center data (masked)',
    level: 1,
  },
];
...
const PERMISSIONS = [
  ...
  { name: 'Request Data', code: 'DATA_REQUEST_CREATE', resource: 'data_request', action: 'create', description: 'Request access to data' },
];
...
async function seedRolesAndPermissions() {
  ...
  try {
    ...
    const dataEntryPermissions = allPermissions.filter(p =>
      (p.resource === 'patients' || p.resource === 'medical_records' || p.resource === 'diagnosis' || p.resource === 'treatments' || p.resource === 'vital_signs') &&
    ...
    // HEAD_OF_CENTER - Input, View, Request Data
    const headOfCenter = await prisma.role.findUnique({ where: { code: 'HEAD_OF_CENTER' } });
    if (headOfCenter) {
      const headPermissions = allPermissions.filter(p =>
        (p.resource === 'patients' && (p.action === 'create' || p.action === 'read' || p.action === 'update' || p.action === 'read_all')) ||
        (p.resource === 'medical_records' && (p.action === 'create' || p.action === 'read' || p.action === 'update')) ||
        (p.resource === 'diagnosis' && (p.action === 'create' || p.action === 'read' || p.action === 'update')) ||
        (p.resource === 'treatments' && (p.action === 'create' || p.action === 'read' || p.action === 'update')) ||
        (p.resource === 'analytics' && p.action === 'read') ||
        (p.resource === 'data_request')
      );
      for (const permission of headPermissions) {
        await prisma.rolePermission.create({
          data: {
            roleId: headOfCenter.id,
            permissionId: permission.id,
          },
        });
      }
    ...
    // OBSERVER - View Only (Masked)
    const observer = await prisma.role.findUnique({ where: { code: 'OBSERVER' } });
    if (observer) {
      const observerPermissions = allPermissions.filter(p =>
  ...
  try {
    ...
    for (const demoUser of demoUsers) {
      ...
      if (!existingUser) {
        ...
        console.log(`ℹ️ Demo user exists, updating role: ${demoUser.email}`);
      }
      ...
      if (demoUser.role) {
        ...
        if (role) {
          ...
          if (!existingUserRole) {
            ...
            console.log(`   └─ Role assigned: ${demoUser.role}`);
          }
        }
      }
    }
