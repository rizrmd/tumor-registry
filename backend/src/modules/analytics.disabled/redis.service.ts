...
export class RedisService implements OnModuleInit, OnModuleDestroy {
  ...
  async getCacheMetrics(): Promise<any> {
    ...
    try {
      const hits = Number(await this.get('metrics:cache_hits') || 0);
      const misses = Number(await this.get('metrics:cache_misses') || 0);
      const queryHits = Number(await this.get('metrics:query_cache_hits') || 0);
      const queryMisses = Number(await this.get('metrics:query_cache_misses') || 0);
