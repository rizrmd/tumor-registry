...
export class DatabasePerformanceService implements OnModuleInit {
  ...
  private async initializeDatabaseMonitoring() {
    try {
      // Enable query performance tracking
      await this.prisma.$executeRaw`CREATE EXTENSION IF NOT EXISTS pg_stat_statements;`;

      // Create custom performance views if they don't exist
  ...
  private async createPerformanceViews() {
    try {
  ...
  async collectDatabaseMetrics(): Promise<DatabasePerformanceMetrics> {
  ...
  private async getQueryPerformanceStats() {
    try {
      const result = await this.prisma.$queryRaw`
        SELECT
  ...
  private async getCachePerformanceStats() {
    try {
      const result = await this.prisma.$queryRaw`
        SELECT
  ...
  private async checkTableBloat() {
    try {
      ...
      this.logger.debug(`Table bloat check completed. Found ${(bloatedTables as any[]).length} tables`);
    } catch (error) {
  ...
  private async refreshMaterializedViews() {
    try {
      // Refresh materialized views that might exist
      await this.prisma.$executeRaw`
        DO $$
        BEGIN
  ...
  private checkDatabaseAlerts(metrics: DatabasePerformanceMetrics) {
    const alerts = [];
  ...
  // Public API methods
  getMetricsHistory(hours: number = 24): DatabasePerformanceMetrics[] {
