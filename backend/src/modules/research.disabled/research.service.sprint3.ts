...
export class ResearchSprint3Service {
  ...
  async createResearchRequest(requestData: any, userId: string) {
    try {
      ...
      await this.auditLogService.log(
        ...
        userId,
        { resource: 'research_request', resourceId: request.id, title: request.title, studyType: request.studyType }
      );
  ...
  async updateCollaborationStatus(id: string, status: CollaborationStatus, userId: string) {
    try {
      ...
      await this.auditLogService.log(
        ...
        userId,
        { resource: 'research_collaboration', resourceId: id, status }
      );
  ...
  async endDataAccessSession(id: string, endTime: Date, dataAccessed: any, queriesExecuted: any, userId: string) {
    try {
      ...
      if (complianceResult.status !== DataAccessComplianceStatus.COMPLIANT) {
        await this.handleComplianceViolation(updatedSession, complianceResult);
      }

      await this.auditLogService.log(
        'END_DATA_ACCESS_SESSION',
        userId,
        {
          resource: 'data_access_session',
          resourceId: id,
  ...
  private async initializeApprovalWorkflow(researchRequestId: string, userId: string) {
    ...
    for (const level of requiredLevels) {
      await this.prisma.researchApproval.create({
        data: {
          researchRequestId,
          approverId: userId, // Placeholder - should be determined by role
          level,
  ...
  private async checkApprovalPermission(level: ApprovalLevel, userId: string) {
    ...
    const hasPermission = user.userRoles.some(userRole =>
      userRole.role.permissions.some(rolePermission =>
        rolePermission.permission.code === `APPROVE_${level}` || rolePermission.permission.code === 'ADMIN_ALL'
      )
    );
  ...
  private async checkUserPermission(userId: string, permissionCode: string): Promise<boolean> {
    ...
    return user.userRoles.some(userRole =>
      userRole.role.permissions.some(rolePermission =>
        rolePermission.permission.code === permissionCode || rolePermission.permission.code === 'ADMIN_ALL'
      )
    );
  }
  ...
  private async performComplianceCheck(session: any, dataAccessed: any, queriesExecuted: any): Promise<{ status: DataAccessComplianceStatus; violationReason: string | null }> {
    // Automated compliance checking logic
    let status: DataAccessComplianceStatus = DataAccessComplianceStatus.COMPLIANT;
    ...
    if (accessCount > 1000) {
      status = DataAccessComplianceStatus.WARNING;
    ...
    if (dataAccessed && JSON.stringify(dataAccessed).includes('SSN')) {
      status = DataAccessComplianceStatus.VIOLATION;
  ...
  private async notifyNewResearchRequest(request: any) {
    ...
    await this.emailService.sendNotificationEmail(
      'admin@inamsos.go.id',
      `New Research Request: ${request.title}`,
      `A new research request has been submitted: ${request.description}`
    );
  }

  private async notifyApprovalUpdate(approval: any) {
    // Send notification to research team about approval status
    const email = approval.researchRequest?.creator?.email || 'admin@inamsos.go.id';
    await this.emailService.sendNotificationEmail(
      email,
      `Approval Update: ${approval.researchRequest?.title || 'Research Request'}`,
      `Your research request has an approval update. Status: ${approval.status}`
    );
  }

  private async sendCollaborationInvitation(collaboration: any) {
    // Send invitation email to collaborator
    const email = collaboration.collaborator?.email || collaboration.email || 'admin@inamsos.go.id';
    await this.emailService.sendNotificationEmail(
      email,
      `Research Collaboration Invitation: ${collaboration.researchRequest?.title || 'Research Request'}`,
      `You have been invited to collaborate on a research project. Role: ${collaboration.role}`
    );
  }

  private async notifyCollaborationStatusUpdate(collaboration: any) {
    // Notify research team about collaboration status update
    const email = collaboration.researchRequest?.creator?.email || 'admin@inamsos.go.id';
    await this.emailService.sendNotificationEmail(
      email,
      `Collaboration Status Update: ${collaboration.collaborator?.name || 'Collaborator'}`,
      `Collaboration status has been updated to: ${collaboration.status}`
    );
  }
  ...
  async updateResearchRequest(id: string, updateData: any, userId: string) {
    try {
      ...
      const allowedStatuses: ResearchRequestStatus[] = [ResearchRequestStatus.DRAFT, ResearchRequestStatus.PENDING_REVIEW];
  ...
  async getImpactMetricsByRequestId(researchRequestId: string, searchDto: any, userId: string) {
    try {
      // Check access to research request
      const researchRequest = await this.prisma.researchRequest.findUnique({
        where: { id: researchRequestId },
      ...
      const [
        ...
        this.prisma.dataAccessSession.count({
          where: {
            ...
            complianceStatus: { in: [DataAccessComplianceStatus.VIOLATION, DataAccessComplianceStatus.WARNING] },
          },
        }),
      ]);
  ...
  async validateResearchRequestData(requestData: any, userId: string) {
    try {
  ...
  private async getComplianceRate(fromDate: Date) {
    const [total, compliant] = await Promise.all([
      ...
      this.prisma.dataAccessSession.count({
        where: {
          startTime: { gte: fromDate },
          complianceStatus: DataAccessComplianceStatus.COMPLIANT,
        },
      }),
    ]);
