...
export class SessionManagementService {
  ...
  async createSession(userId: string, ipAddress: string, userAgent: string, token: string) {
    ...
    const session = await this.prisma.userSession.create({
      data: {
        ...
        // deviceName: `${ua.browser.name} on ${ua.os.name}`, // Field doesn't exist in Prisma schema
        // browser: ua.browser.name, // Field doesn't exist in Prisma schema
        // os: ua.os.name, // Field doesn't exist in Prisma schema
        deviceFingerprint,
  ...
  async terminateAllSessions(userId: string, exceptSessionId?: string) {
    ...
    await this.prisma.auditLog.create({
      data: {
        ...
        resource: 'SESSION',
        details: { exceptSessionId, userId },
        ipAddress: '',
        userAgent: '',
      },
    });
  ...
  async detectSessionAnomaly(userId: string, newSession: any) {
    ...
    if (anomalies.length > 0) {
      // Create security alert - Temporarily disabled for MVP
      // await this.prisma.securityAlert.create({
      //   data: {
      //     userId,
      //     // type: 'SESSION_ANOMALY', // Field doesn't exist in SecurityAlert
      //     severity: 'MEDIUM',
      //     description: `Session anomalies detected: ${anomalies.join(', ')}`,
      //     // details: { // Field doesn't exist in SecurityAlert
      //     //   anomalies,
      ...
      // Send notification - Temporarily disabled for MVP
