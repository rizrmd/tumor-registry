    ...
    try {
      ...
      const latestRecord = await this.prisma.medicalRecord.findMany({
        where: {
          ...
          recordType,
          recordNumber: {
            startsWith: `${patient.hospitalRecordNumber}-${recordType}-${currentYear}${currentMonth}`,
          },
        },
        orderBy: {
          recordNumber: 'desc',
      ...
      return `${patient.hospitalRecordNumber}-${recordType}-${currentYear}${currentMonth}-${sequence.toString().padStart(3, '0')}`;
    } catch (error) {
