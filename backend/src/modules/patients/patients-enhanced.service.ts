...
export class PatientsEnhancedService {
  ...
  async create(createDto: CreatePatientDto) {
    try {
      ...
      // Validate MRN uniqueness (Hospital Record Number)
      if (createDto.hospitalRecordNumber) {
        ...
        if (existingMrn) {
          throw new ConflictException('Patient with this Hospital Medical Record Number already exists');
        }
      }
      ...
      // Generate INAMSOS Record Number
      ...
      const patient = await this.prisma.patient.create({
        data: {
          ...
          anonymousId: `P-${createDto.centerId}-${inamsosRecordNumber.split('-')[2]}`, // Simple anonymous ID for now
