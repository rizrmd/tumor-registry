// HL7/FHIR and External System Integration
...
export interface TransformationRule {
  ...
  inputType: string;
  outputType: string;
  script?: string; // JavaScript, Python, or custom transformation script
  function?: string;
  parameters?: Record<string, any>;
}
...
export interface HL7Message {
  id: string;
  messageType: string; // ADT, ORU, PID, etc.
  triggerEvent: string; // A01, A04, etc.
...
export interface HL7Segment {
  segmentType: string; // MSH, PID, ORC, etc.
  fields: HL7Field[];
  rawSegment: string;
}
...
export interface FHIRResource {
  ...
  resource: any; // The actual FHIR resource object
...
// Integration Events and Workflows
...
export interface MappingRule {
  id: string;
  sourcePath: string; // HL7 segment.field or FHIR path
  targetPath: string;
  dataType: string;
  required: boolean;
  defaultValue?: any;
